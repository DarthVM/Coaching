# Теоретическая часть
**Библиотека** - это отдельный програмный модуль, содержащий в себе готовые реализации функций и структур данных, которые используют другие программы.



## Виды библиотек в C/C++
...
Разница заключается в способе подключения. Схемы ниже представляют упрощенные этапы компиляции программы на C/C++ (для простоты расширения файлов как в Windows).
### Статическая бибилотека
![alt text](/Assets/Static%20Library%20Linking.png)

Подключение статической библиотеки происходит на этапе компиляции, здесь все функции и структуры данных копируются в исполняемый файл, так как это уже скомпилированный в машинный код файл с особым расширением.



### Динамическая библиотека

![alt text](/Assets/Dynamic%20Library%20Linking.png)

Динамическая библиотека подключается во время исполнения программы (в Рантайме - Runtime), т. е. вместо копировании всего машинного кода, передается ссылка на отдельный файл библиотеки и, по мере необходимости, при вызове определенной функции управление передается библиотеке, которая исполняет функцию.
...
|:------:|:----------------------------------------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
|  Плюсы | Нет тех минусов, что в динамической библиотеке |                                                             1. Может использоваться несколькими программами<br>2. Загружается по мере необходимости                                                             |
| Минусы |          Увеличение размера программы          | 1. Если изменить сигнатуру функции в библиотеке, то может необходимо перекомпилировать программу, которая ее использует<br>2. Если функций много, то уйдет много времени, чтобы найти точку входа конкретной функции |

____________________________

# Работа с динамической библиотеки (DLL) в C++. 

### Предварительная настройка

* Требуется ```CMake``` [Ссылка на скачку тут](https://github.com/Kitware/CMake/releases/download/v4.1.0/cmake-4.1.0-windows-x86_64.msi)
* Для компиляции без CMake и в обычной командной строке
...
Помимо подключения библиотеки на этапе запуска программы с ```.dll```, можно подключить библиотеку прямо в коде без компиляции с ```.lib``` файлами.
...
### Особенности
* MinGW (GCC) формирует не .lib, а .dll.a, т. е. архивированный файл библиотеки импорта, но разница в подключении нет и можно считать альтернативой.
...
Встроенная библиотека ```ctypes``` позволяет явно подключить DLL в Python.
...
my_library = cdll.LoadLibrary("path/to/my_lib_dll")
add_result = my_library.add_numbers(2, 4)

print(add_result)
```


Подробнее [тут](https://docs.python.org/3/library/ctypes.html#)